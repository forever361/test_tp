    ...
    def __init__(self,
        ...
        if log_dir:
            file_handler = TimedRotatingFileHandler(
                filename=os.path.join(log_dir, 'tanos.log'),
                when='D',
                interval=1,
                backupCount=30  # 保留7天的日志文件
            )
            file_handler.suffix = "%Y-%m-%d_%H-%M-%S.log.gz"
