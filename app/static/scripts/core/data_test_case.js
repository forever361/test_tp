...
oTableInit.Init = function () {
    $('#tb_test_cases_data').bootstrapTable({
        url: '/data_test_case.json',         //请求后台的URL（*）
        method: 'get',                      //请求方式（*）
...
//            height: 600,                        //行高，如果没有设置height属性，表格自动根据记录条数觉得表格高度
        ...
        columns: [{
            ...
            formatter: function (value, row, index) {
                var edit = '<a href="javascript:;" onclick="window.location.href=(\'/data_edit_test_case?id='+ row.id + '\')">'+value+'</a> ';
                return edit;
            }
         }, {
            field: 'operate',
            ...
            align: 'center',
            formatter: function (value, row, index) {
                    var a = '<a href="javascript:;" onclick="window.location.href=(\'/data_edit_test_case?id='+ row.id + '\')">EDIT</a> ';
...
function edit(index) {
    window.location.href=('/data_edit_test_case?id='+index);
}
...
function searchTestCase(test_case_id){
    var $tb_departments = $('#tb_test_cases_data');
    $tb_departments.bootstrapTable('refresh', {url: '/data_test_case.json',data:{id: test_case_id,type:"test_case"}});
}
