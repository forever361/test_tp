...
function get_test_case_detail(id){
  ...
  dataType: dataType
    ...
    var web_oTableInit = new Object();
    //初始化Table
    oTableInit.Init = function () {
        $('#tb_test_cases_web').bootstrapTable({
            url: '/web_test_case.json',         //请求后台的URL（*）
            method: 'get',                      //请求方式（*）
            ...
            columns: [{
                ...
                formatter: function (value, row, index) {
                        var a = '<a href="javascript:;" onclick="window.location.href=(\'/web_edit_test_case?id='+ row.id + '\')">EDIT</a> ';
                        //var b = '<a href="javascript:;" onclick="copy_test_case(\'' + row.id + '\')">复制</a> ';
//                        var c = '<a href="javascript:;" onclick="run_test_case(\'' + row.id + '\')">执行</a> ';
                        var e = '<a href="javascript:;" onclick="window.location.href=(\'/web_search_report?id='+ row.id +'\')">RESULT</a> ';
                        var d = '<a href="javascript:;" onclick="delete_test_case(\'' + row.id + '\')">DELETE</a> ';
                        return a+d+e;
                        }
                }
                ]
    ...
    function edit(index) {
        window.location.href=('/web_edit_test_case?id='+index);
    }
...
function searchTestCase(test_case_id){
    var $tb_departments = $('#tb_test_cases_web');
    $tb_departments.bootstrapTable('refresh', {url: '/web_test_case.json',data:{id: test_case_id,type:"test_case"}});
}
...
function get_edit_info(active_id)

  {
    ...
    $.ajax(
        {
          url: "/test_case.json",
          data:{"id":active_id,"type":"case_one"},
          type: "get",
          dataType:"json",
          beforeSend:function()
          {
...
//              var data_obj = eval("("+data_obj0+")");
              ...
              // 赋值 左边是网页的元素id 右边括号里就是最后在网页呈现的值,右边的值是从json中获取的
//              $("#id").val(id);
//              $("#name").val(data_obj.name);
//              $("#user_id").val(data_obj.user_id);
              $("#code").val(data_obj.testinfor);
...
function delete_test_case(active_id)
  ...
  {
     layer.confirm("Confirm to delete?",{btn:['Yes','No'],tile:"123"},function(){
          if(!active_id)
          {
            alert('Error！');
            return false;
         ...
         $.ajax(
          {
            ...
            success:function(data)
            {
              if(data.code = 200)
              {
                layer.msg('Delete success!',{time:1500});
                // common_ops.alert('恭喜，删除成功！');
         ...
         $.ajax(
        {
          ...
          success:function(data)
          {
            if(data)
            {
              ...
              if(data.code==200){
//              alert('success!');
              window.location.href=('/edit_test_case?id='+ test_case_id  )
              }else{
