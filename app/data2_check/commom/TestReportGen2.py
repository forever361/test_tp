...
sys.path.append('..')
from app.data_check.Common.write_excel_data import ExcelUtilAll
...
class Template_mixin(object):
    ...
    def __init__(self, verbosity=1, retry=0, save_last_try=False):
        ...
        self.description = "Checking count and value of database Result:"
        self.run_times = 0
        ...
        #以下参数都是从excel读取，准备写入htmt
        self.TD_Tablename = ExeHandle.get_col_value(None, 2, 1) #1
        self.MX_Tablename = ExeHandle.get_col_value(None, 2, 2) #2
        # print(2222222222,self.MX_Tablename )
        self.TD_Count = ExeHandle.get_col_value(None, 2, 3) #3
        # self.TD_Count = TD_Count.split(":")[1].strip("}")
        self.result.append(self.TD_Count)
        self.MX_Count = ExeHandle.get_col_value(None, 2, 4) #4
        # self.MX_Count = MX_Count.split(":")[1].strip("}")
        self.result.append(self.TD_Count)
        self.Count_Pass = ExeHandle.get_col_value(None, 2, 5) #5
        self.result.append(self.Count_Pass)
        self.Count_fail = ExeHandle.get_col_value(None, 2, 8)
        self.result.append(self.Count_fail)
        self.Value_Pass = ExeHandle.get_col_value(None, 2, 6) #6
    ...
    .MX_CountClass  { background-color: #6c6;  !important ;}
    .Count_PassClass  { background-color: #6c6;  !important ;}
    .Count_failClass  { background-color: #c60;  !important ;}
    ...
    .Value_failCase  { color:#908e8e; font-weight: bold; }
    ...
    .detail_button.TD_Passed{  background-color: #66cc66;}
    .detail_button.MX_Passed{  background-color: #66cc66;}
    .detail_button.Count_Passed{  background-color: #66cc66;}
    .detail_button.Count_failed{  background-color: #cc6600;}
    .detail_button.Value_Passed{ background-color: #66cc66;}
    .detail_button.Value_failed{ background-color: #cc6600;}
    ...
    def _generate_report(self):
        ...
        <tr id='total_row'>
            ...
            <th>%(Count_Pass)s</th>
            <th>%(Value_Pass)s</th>
            <th><a href="http://{}:{}/static/userinfo/{}/{}_{}.csv" target="_blank" style="color:white"> Click:AttachmentLink >>> </tr>
        </table>
        <script>
            showCase(0,%(channel)s);
            drawCircle('circle%(channel)s',%(Count_Pass)s, %(Count_fail)s, %(Value_Pass)s,  %(Value_fail)s);
        </script>
        """.format(Constant().ip, Constant().port, userid,self.MX_Tablename, caseid)
        # print(111111,self.MX_Tablename)

        report = REPORT_TMPL % dict(
            test_list=u''.join(rows_report),
            Oracle_Tablename=str(self.MX_Tablename),
            Max_Tablename=str(self.MX_Tablename),
            Tablename=str(self.MX_Tablename),
            TD_Count=str(self.TD_Count),
            MX_Count=str(self.MX_Count),
            count=str(self.total),
            Count_Pass=str(self.Count_Pass),
            Count_Pass_p=1 if self.total else 0.0,
    ...
    def getReportAttributes(self):
        ...
        # if self.Count_fail:
        #     status.append(u'<span class="tj Count_failCase">Count_fail</span>:%s' % self.Count_fail)
        # if self.Value_Pass:
    ...
    def Data_html(self):
        ...
        # print ('开始新的测试报告页面。。。。')
        with open(userPath + '{}_data_test.html'.format(caseid), 'w', encoding="utf-8") as f:
            f.write(
                Template_mixin.html_tmp % {'TD_Count': self.TD_Count, 'MX_Count': self.MX_Count, 'total': self.total,
                                           'count': self.count, 'Count_Pass_p': self.Count_Pass_p,
                                           'Count_Pass': self.Count_Pass,
                                           'Count_fail': self.Count_fail, 'Value_Pass': self.Value_Pass,
                                           'Value_fail': self.Value_fail,
                                           'channel': channel, 'test_list': test_list, 'heading': heading,
                                           'title': self.title,
                                           'generator': generator, 'stylesheet': stylesheet, 'report': report,
                                           'ending': ending})
            f.close()
