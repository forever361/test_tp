...
basepath_common = os.path.abspath(os.path.join(os.path.dirname(__file__), "/Common"))

userid = ConnectSQL().get_personal_user_id()
userPath = os.path.join(basepath+"/templates/userinfo/{}/".format(userid))

reportPath = os.path.join(basepath+"/templates/")
imgPath = os.path.join(basepath+"/static/")
sys.path.append(basepath)
sys.path.append(basepath_common)
sys.path.append('..')
from app.data2_check.commom.write_excel_data import ExcelUtilAll
logPath = os.path.join(basepath+"/Log/")
# Log = MyCommon()
ExeHandle = ExcelUtilAll()
...
class Template_mixin(object):
    def __init__(self,verbosity=1,retry=0,save_last_try=False):
        ...
        self.description = "Check count and value of database"
        self.run_times = 0
        ...
        self.TD_Tablename = ExeHandle.get_col_value(None, 2, 1)
        self.MX_Tablename = ExeHandle.get_col_value(None, 2, 2)

        self.TD_Count = ExeHandle.get_col_value(None, 2, 5)
        self.result.append(self.TD_Count)
        self.MX_Count = ExeHandle.get_col_value(None, 2, 6)
        self.result.append(self.MX_Count)
        self.Count_Pass = ExeHandle.get_col_value(None, 2, 7)
        self.result.append(self.Count_Pass)
        self.Count_fail = ExeHandle.get_col_value(None, 2, 8)
        self.result.append(self.TD_Count)
        self.Value_Pass = ExeHandle.get_col_value(None, 2, 9)
        self.result.append(self.Value_Pass)
        self.Value_fail = ExeHandle.get_col_value(None, 2, 10)
        self.result.append(self.Value_fail)
        self.Count_Pass_p = self.Count_Pass
        self.total = self.Count_Pass + self.Count_fail
        self.count = self.Count_Pass + self.Count_fail
    ...
    """
    # variables: (test_title, count, Count_Pass, Count_fail, Value_Pass, Value_fail)
    ...
    .TD_CountClass  { background-color: #6c6;  !important ;}
    .MX_CountClass  { background-color: #c60;  !important ;}
    .Count_PassClass { background-color: #c00; !important ; }
    .Count_failClass { background-color: #c00; !important ; }
    .Value_PassClass { background-color: #c00; !important ; }
    .Value_failClass { background-color: #c00; !important ; }
    .Count_PassCase   { color: #6c6; }
    .Count_failCase   { color: #c60; font-weight: bold; }
    .Value_PassCase  { color: #c00; font-weight: bold; }
    .Value_failCase  { color: #c00; font-weight: bold; }
    tr[id^=ct]  td { background-color: rgba(73,204,144,.3) !important ; }
    tr[id^=vt]  td { background-color: rgba(73,204,144,.3) !important; }
    tr[id^=pt]  td { background-color: rgba(73,62,62,.3) !important ; }
    tr[id^=ft]  td { background-color: rgba(252,62,62,.3) !important ; }
    ...
    .detail_button.TD_Passed{background-color: #4dbee8;}
    .detail_button.MX_Passed{background-color: #4dbee8;}
    .detail_button.Count_Passed{background-color: #4dbee8;}
    .detail_button.Count_failed{background-color: #4dbee8;}
    .detail_button.Value_Passed{background-color: #4dbee8;}
    .detail_button.Value_failed{background-color: #4dbee8;}
    ...
    #     <img src="http://{}:{}/templates/userinfo/{}/td_mx_check.png">
    # </div>
    # <div id="td_mx_check" class="td_mx_check" >
    #     <img src="http://{}:{}/templates/userinfo/{}/d_mx_check.png">
    # </div>
    #
    # <div onmousemove="bigImgVal(this)" onmouseout="normalImgVal(this)" class="report_pie_value">
    #     <img src="http://{}:{}/templates/userinfo/{}/td_mx_check_value.png">
    # </div>
    # <div id="td_mx_check_value" class="td_mx_check_value" >
    #     <img src="http://{}:{}/templates/userinfo/{}/td_mx_check_value.png">
    # </div>
    #
    # """.format(Constant().ip,Constant().port,userid,Constant().ip,Constant().port,userid,Constant().ip,Constant().port,userid,Constant().ip,Constant().port,userid)
    ...
    <div id='show_detail_line' style=" float: left; width: 100%%;">
        <a class="TD_Passed detail_button" href='javascript:showCase(1,%(channel)s)'>TD_Count[%(TD_Count)s]</a>
        <a class="MX_Passed detail_button" href='javascript:showCase(1,%(channel)s)'>MX_Count[%(TD_Count)s]</a>
        <a class="Count_Passed detail_button" href='javascript:showCase(1,%(channel)s)'>Count_Pass[%(Count_Pass)s]</a>
        <a class="Count_failed detail_button" href='javascript:showCase(1,%(channel)s)'>Count_fail[%(Count_fail)s]</a>
        <a class="Value_Passed detail_button" href='javascript:showCase(1,%(channel)s)'>Value_Pass[%(Value_Pass)s]</a>
        <a class="Value_failed detail_button" href='javascript:showCase(1,%(channel)s)'>Value_fail[%(Value_fail)s]</a>
    </div>
    ...
    <tr id='header_row'>
        ...
        <th>Value_fail</th>
        <th>view</th>
        <th>Value_fail screenshot</th>
    </tr>
    %(test_list)s
    <tr id='total_row'>
        <th>%(TD_Tablename)s / %(MX_Tablename)s</th>
        <th>%(TD_Count)s</th>
        <th>%(MX_Count)s</th>
        <th>%(Count_Pass)s</th>
        <th>%(Count_fail)s</th>
    ...
    <script>
        ...
        drawCircle('circle%(channel)s', %(Count_Pass)s, %(Count_fail)s, %(Value_Pass)s, %(Value_fail)s);
    </script>
    """
...
"""  # variables: (tid, Class, style, desc, status,img)
    ...
    def DrawPie_Count(self):
        """
        绘制饼图用pie
        """
        labels = 'Count_Pass', 'Count_fail'
        params = [self.Count_Pass, self.Count_fail]
        colors = ['green', 'orange']
        explode = [0, 0]  # 0.1凸出这部分
        plt.axes(aspect=1)  # set this, Figure is round, otherwise it is an ellipse.
        # autopct, show percet
        plt.pie(x=params, colors=colors, labels=labels, explode=explode, autopct='%3.1f %%', normalize=True,
                shadow=True, labeldistance=1.1, startangle=80, pctdistance=0.6)
        # plt.show()
        # 显示图例
        plt.legend()
        # commonPath = Log.get_Common()
        imgPath1 = os.path.join(userPath, "td_mx_check.png")
        print("imgPath:::::",imgPath1)
        plt.savefig(imgPath1)
        plt.close()


    def DrawPie_Value(self):
        """
        绘制饼图用pie
        """
        labelss = 'Value_Pass', 'Value_fail'
        paramss = [self.Value_Pass, self.Value_fail]
        colorss = ['green', 'orange']
        explodes = [0, 0]  # 0.1凸出这部分
        plt.axes(aspect=1)  # set this, Figure is round, otherwise it is an ellipse.
        # autopct, show percet
        plt.pie(x=paramss, colors=colorss, labels=labelss, explode=explodes, autopct='%3.1f %%', normalize=True,
                shadow=True, labeldistance=1.1, startangle=80, pctdistance=0.6)
        # plt.show()
        # 显示图例
        plt.legend()
        imgPath1 = os.path.join(userPath, "td_mx_check_value.png")
        plt.savefig(imgPath1)
        plt.close()
    ...
    def run(self):
        ...
        self.stopTime = datetime.datetime.now()
        self.run_times += 1
        return self.startTime,self.stopTime,self.run_times
    ...
    def _generate_report(self):
        rows_report = []
        report = Template_mixin.REPORT_TMPL % dict(
            test_list=str(u''.join(rows_report)),
            TD_Tablename=str(self.TD_Tablename),
            MX_Tablename=str(self.MX_Tablename),
            TD_Count=str(self.TD_Count),
            MX_Count=str(self.MX_Count),
            count=str(self.total),
            Count_Pass=str(self.Count_Pass),
            # Count_Pass_p=self.Count_Pass*1.00/self.total*100 if self.total else 0.0,
            Count_Pass_p="1",
            Count_fail=str(self.Count_fail),
            Value_Pass=str(self.Value_Pass),
            Value_fail=str(self.Value_fail),
    ...
    def generateReport(self):
        ...
        output = Template_mixin.html_tmp % dict(
            title=saxutils.escape(self.title),
            generator=generator,
    ...
    def getReportAttributes(self):
        ...
        """
        # self.startTime = WritePlt.run(self)
        # self.stopTime = WritePlt.run(self)
        # startTime = str(self.startTime)[:19]
        # duration = str(self.stopTime - self.startTime)
        startTime = self.startTime.strftime("%Y-%m-%d %H:%M:%S")
        # status = []
        # if self.Count_Pass:
        #     status.append(u'<span class="tj Count_PassCase">Count_Pass</span>:%s' % self.Count_Pass)
        # if self.Count_fail:
        #     status.append(u'<span class="tj Count_failCase">Count_Pass</span>:%s' % self.Count_fail)
        # if self.Value_Pass:
        #     status.append(u'<span class="tj Value_PassCase">Value_Pass</span>:%s' % self.Value_Pass)
        # if self.Value_fail:
        #     status.append(u'<span class="tj Value_failCase">Value_fail</span>:%s' % self.Value_fail)
        # self.total_count = self.Count_Pass + self.Count_fail
        # self.total_value = self.Value_Pass + self.Value_fail
        # self.total = self.Count_Pass + self.Count_fail + self.Value_Pass + self.Value_fail
        # if self.total_count or self.total_value >0:
        #     Count_Passed = self.Count_Pass * 1.000 / self.total_count * 100
        #     Value_Passed = self.Value_Pass * 1.000 / self.total_value * 100
        # else:
        #     Count_Passed = 0.0
        #     Value_Passed = 0.0
        # status.append(u'<br/>')
        # status.append(u'<span class="tj">Count_Passrate</span>:%.1f%%' % Count_Passed)
        # status.append(u'<span class="tj">Value_Passrate</span>:%.1f%%' % Value_Passed)
        # if status:
    ...
    def _generate_ending(self):
        return Template_mixin.ENDING_TMPL
    ...
    def Data_html(self):
        ...
        with open(userPath + '10217_data_test.html','w', encoding="utf-8") as f:
            f.write(
                Template_mixin.html_tmp % {'TD_Count':self.TD_Count,'MX_Count':self.MX_Count,'total':self.total,
                                           'count':self.count,'Count_Pass_p':self.Count_Pass_p,'Count_Pass':self.Count_Pass,
                                           'Count_fail':self.Count_fail,'Value_Pass':self.Value_Pass,'Value_fail':self.Value_fail,
                                           'channel':channel,'test_list':test_list,'heading':heading,
                                           'title':self.title,'generator':generator,'stylesheet':stylesheet,
                                           'report':report,'ending':ending})
            f.close()


if __name__ == "__main__":
    write = Template_mixin()
    write.Data_html()