...
@web.route('/api_batch_test_data_f2t',methods=['POST'])
def batch_test_compare1():
    ...
    if 'Save' in request.form:
        ...
        rows= ConnectSQL().data_f2t_get_infor_value(case_name)
        print ('rows:',rows)
        ...
        if  rows == [] or rows == None:
            ...
            ConnectSQL().data_f2t_write_config_value_all(user_id, case_name, code, code1)

            case_id = ConnectSQL().data_f2t_get_case_id(case_name)[0][0]
            configP2.clear()
            ...
            return render_template('code_mode/data_test_compare_save_f2t.html'  ,
...
log_data = []
# 定义接口把处理日志并返回到前端
@web.route('/data_f2t_get_log',methods=['GET','POST'])
def get_log_f2t(flag=False):
    if flag==False:
        log_data = red_logs_f2t() # 获取日志
...
@web.route('/clear_log_f2t',methods=['GET','POST'])
def clear_log():
    get_log_f2t(flag=True)
    return ''
